...
export class ViewPaneComponent implements OnInit, OnDestroy, AfterViewInit, AfterViewChecked {
        ...
        if (overlayVideo.videoWidth > 0 && overlayVideo.videoHeight > 0) {
          ...
          if (touchTarget) {
            ...
            if (!this.compositeDebugLogged) {
              ...
              console.log('[Render Debug] Overlay:', { oX, oY, oW, oH });
              console.log('[Render Debug] Canvas:', { w: canvas.width, h: canvas.height, scaleY: canvasRect.height / canvas.height });
            }
          }
        }
      }
  ...
  private setupOverlayTouchHandling(): void {
    ...
    const handler = (e: PointerEvent) => {
      ...
      // touch.js reads e.offsetX/e.offsetY and scales them by
      // (videoWidth / offsetWidth). For cross-frame synthetic events,
      // Chrome does NOT reliably compute offsetX/offsetY from clientX/clientY.
      // So we set offsetX/offsetY explicitly to the values touch.js expects.
      const targetOffsetX = relX * video.offsetWidth;
      const targetOffsetY = relY * video.offsetHeight;

      const videoRect = video.getBoundingClientRect();
      const clientX = videoRect.left + targetOffsetX;
      const clientY = videoRect.top + targetOffsetY;
